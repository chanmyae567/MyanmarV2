...
|     |  |  |  _  |   | |     |  _  | __  |
| | | |_   _|     | | | | | | |     |    -|
|_|_|_| |_| |__|__|_|___|_|_|_|__|__|__|__| { V.2 }
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] MODIFIED  : LTKYAW
 [\x1b[1;96m+%s] COUNTRY   : MYANMAR
 [\x1b[1;96m+%s] CRACK PRO : 2022 >> TOOL
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] Success login %s'%(H,N,cc))
				self.bot()
			except KeyError:
				jalan(' [%s!%s] Token error try again other accoun!'%(M,N))
				self.takon()
	def takon(self):
...
class menu:
	...
	def main(self):
		...
		jalan(' %s╔═[ %sWELCOME >< %s%s ]'%(N,H,r,N))
		print(' %s║'%(N))
		print(' %s╠═[%s•%s]  GOODLUCK  : %s'%(N,O,N,akss))
		print(' %s╠═[%s•%s] YOUR CURRENT IP ADDRESS : %s'%(N,O,N,IP))
		print(' %s╠═[%s•%s] YOUR LOGGED IN          : %s'%(N,O,N,waktu))
		print(' %s║'%(N))
		print(' %s╠═[%s0%s] CRACK FRIEND LIST          [😑]'%(N,O,N))
		print(' %s╠═[%s1%s] CRACK PUBLIC ACCOUNT       [🥰]'%(N,O,N))
		print(' %s╠═[%s2%s] CRACK PUBLIC MANY ACCOUNT  [😍]'%(N,O,N))
		print(' %s╠═[%s3%s] CRACK FROM POSTING         [😅]'%(N,O,N))
		print(' %s╠═[%s4%s] CRACK FROM LIKE POST       [😂]'%(N,O,N))
		print(' %s╠═[%s5%s] CRACK FROM FOLLOW          [🤩]'%(N,O,N))
		print(' %s╠═[%s6%s] CHECK CP ACCOUNT           [🥺]'%(N,O,N))
		print(' %s╠═[%s7%s] CHECK CRACK RESULT OK/CP   [😉]'%(N,O,N))
		print(' %s╠═[%s8%s] SETTING USER-AGENT         [😶]'%(N,O,N))
		print(' %s╠═[%s9%s] CRACK EMAIL                [😅]'%(N,O,N))
		print(' %s╠═[%sG%s] GET DATA² FACEBOOK         [😁]'%(N,O,N))
		print(' %s╠═[%sK%s] REPORT BUG SCRIPT          [😆]'%(N,O,N))
		print(' %s╠═[%sA%s] LOGOUT, DELETE TOKEN       [😑]'%(N,O,N))
		print(' %s╠═[%sU%s] UPGRADE TO PREMIUM         [🤣]'%(N,O,N))
		self.pilih()
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				print(' %s║'%(N))
				idt = input(' %s╠═[%s•%s] INPUT ID : '%(N,O,N))
...
class crack:
	...
	def fbeh(self,id):
		...
		kham = input(' %s╠═[%s?%s] USE PASSWORD MANUAL Y/T : '%(N,O,N))
		if kham in ['']:
			...
			while True:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						print('%s ║'%(N))
						mani = input(' %s╠═[%s•%s] METHOD : '%(N,O,N))
						if mani in ['']:
							print(' %s╠═[%s!%s] Jangan kosong mmk'%(N,M,N));self.manual()
						elif mani in ['1','01']:
							print(' %s║'%(N))
							print(' ╠═[%s*%s] OK ACCOUNT WILL BE SAVE IN FILE : Ok.txt'%(O,N))
							print(' ╚═[%s*%s] CP ACCOUNT WILL BE SAVE IN FILE : Cp.txt\n'%(O,N))
	...
	def basic(self):
		print(' %s║'%(N))
		print(' %s╠═[%s*%s] OK ACCOUNT WILL BE SAVE IN FILE : Ok.txt'%(N,O,N))
		print(' %s╠═[%s*%s] CP ACCOUNT WILL BE SAVE IN FILE : Cp.txt'%(N,O,N))
		print(' %s╚═[%s!%s] AIRPLANE MODE 3 SECOND IF NO RESULT\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
